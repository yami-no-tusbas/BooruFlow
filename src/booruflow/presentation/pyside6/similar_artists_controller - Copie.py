...
class SimilarArtistsController(QObject):
    ...
    def start_remote_discovery(self,mode:str)->None:
        ...
        self.discovery_worker=RemoteDiscoveryWorker(self.image_analysis.database,self.root,query,str(self.page.backend.currentData()),mode,self.providers,self.remote_pixels,self.page.only_new.isChecked());self.discovery_worker.progress.connect(self._remote_discovery_progress);self.discovery_worker.completed.connect(self._remote_discovery_done);self.discovery_worker.failed.connect(self._remote_discovery_failed);self.discovery_worker.start();self.page.remote_cancel.setEnabled(True);self.page.remote_status.setText("Recherche des candidats…");self.discovery_task_id=self.task_manager.start("remote_discovery","Découverte d’artistes distants",mode)

    def _remote_discovery_progress(self,value:dict)->None:
        labels={"distinctive_tags":"Tags distinctifs","candidates":"Recherche de candidats","profiles":"Téléchargement / embeddings / profils","finished":"Terminé"};phase=value["phase"];done=int(value.get("processed",value.get("evaluated",0)));total=max(1,int(value.get("artist_total",value.get("filtered",0) or 1)));self.page.remote_progress.setRange(0,total);self.page.remote_progress.setValue(min(done,total));reason=value.get("reason","");text=(f"Phase : {labels.get(phase,phase)}\nTags distinctifs : {value.get('tags',0)} · Requêtes : {value.get('queries',0)} · Posts : {value.get('posts',0)} · Artistes trouvés : {value.get('artists',0)} · Après filtrage : {value.get('filtered',0)} · Images : {value.get('images',0)} · Profils : {value.get('profiles',0)} · Évalués : {value.get('evaluated',0)}"+(f"\nRaison : {reason}" if reason else ""));self.page.remote_status.setText(text);self.task_manager.progress(self.discovery_task_id,done,total,phase,text)
    ...
    def open_gallery(self,candidate:ArtistIdentity)->None:
        ...
        backend=str(self.page.backend.currentData());images=self.service.closest_candidate_images(candidate,backend,item_id=self.current_item_id,query_artist=self.current_artist,query_profile=None if self.active_reference_id else self.query_profile,exclude_item_ids=excluded)
        if not images and self.reference_ids:
            ...
            if answer==QMessageBox.StandardButton.Yes:images=self.service.closest_candidate_images(candidate,backend,query_profile=self.query_profile,limit=12)
    ...
    def compare(self,candidate:ArtistIdentity)->None:
        ...
        elif self.query_profile is not None:data=self.service.compare_query_to_artist(self.query_profile,right);left=None;title=f"Références vs {candidate.tag}";left_count=len(self.reference_ids)
        ...
        for section,first,second in (("Tags source fréquents",left.source_tag_frequency if left else {},right.source_tag_frequency),("WD14 acceptés fréquents",left.accepted_wd14_frequency if left else {},right.accepted_wd14_frequency)):
