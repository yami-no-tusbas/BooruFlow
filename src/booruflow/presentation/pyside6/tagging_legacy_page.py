...
class TaggingLegacyPage(QWidget):
    ...
    def _build_review(self) -> None:
        ...
        self.suggestions.itemDoubleClicked.connect(lambda item: self.tag_search_requested.emit(self.suggestions.item(item.row(), 0).text()))
    ...
    def _start(self) -> None:
        ...
        self.processed_in_session.clear(); self.settings["tagging_query"] = request.query; self.query_saved.emit(request.query); self.start_requested.emit(request)
    ...
    def _open_current_post(self)->None:
        if self.current_post_id:
            self._open_post(self.current_post_id)
    def _selected_observation_id(self):
        column=getattr(self,"_suggestion_id_column",4); row=self.suggestions.currentRow(); item=self.suggestions.item(row,column) if row>=0 else None; token=item.text() if item and item.text() else ""; return int(token) if token.isdigit() else token or None
    def _emit_decision(self,decision:str)->None:
        ...
        if oid is not None and self.accept_button.isEnabled():
            column=getattr(self,"_suggestion_id_column",4); row=self.suggestions.currentRow(); self._pending_fallback_row=row; target=self.suggestions.item(row+1,column) or self.suggestions.item(row-1,column); self._pending_next_id=target.text() if target and target.text() else None
            tokens=[self.suggestions.item(index.row(),column).text() for index in self.suggestions.selectionModel().selectedRows() if self.suggestions.item(index.row(),column)]
            if len(tokens)==1:
                target_kind, target_value = parse_review_row_token(tokens[0])
                self.decision_requested.emit(target_value if target_kind == "observation" else tokens[0], decision)
            else:
                self.decision_requested.emit(tokens,decision)
    ...
    def _copy_and_open(self) -> None:
        value = self._clipboard_text("final")
        if value:
            self._copy(f" {value}")
            self.activity_logged.emit(
                "Clipboard",
                f"{len(value.split())} tags copied (final, append)"
            )
        if self.current_post_id:
            self._open_post(self.current_post_id)
            self._mark_processed_and_advance()
    ...
    def _clipboard_text(self,mode:str)->str:
        _ = mode
        return build_final_tags_clipboard(self.tags_to_add.text().split())
    ...
    def _render_suggestions(self,*_args)->None:
        ...
        if target_id is not None:
            for r in range(self.suggestions.rowCount()):
                if self.suggestions.item(r,4).text()==str(target_id):selected_row=r;break
    ...
    def _set_preview_media(self,path)->None:
        video_source = str(self.current_post.get("file_url", ""))
        is_video = path is not None and path.suffix.casefold() in {".webm", ".mp4", ".mov", ".mkv"}
        is_video = is_video or video_source.casefold().split("?", 1)[0].endswith((".webm", ".mp4", ".mov", ".mkv"))
        self.video_player.stop(); self.video_player.setSource(QUrl()); self.video_error_link.hide()
    ...
    def _video_playback_state_changed(self, *_args)->None:
        self._log_video_diagnostic("playback-state")
    def _video_duration_changed(self, *_args)->None:
        self._log_video_diagnostic("duration")
    def _video_tracks_changed(self, *_args)->None:
        self._log_video_diagnostic("tracks")
    def _log_video_diagnostic(self, event: str, source: QUrl | None = None)->None:
        source = source or self.video_player.source()
        filename = Path(source.path()).name or "remote-media"
        extension = Path(filename).suffix.casefold() or "unknown"
        player = self.video_player
        print(
            "[DEBUG] [LegacyVideo] "
            f"event={event}; media={filename}; extension={extension}; "
    ...
    def _schedule_manual_lookup(self,text:str)->None:
        self.manual_lookup_timer.stop()
        if len(text.strip()) >= 2:self.manual_lookup_timer.start()
        else:self.manual_suggestions.setStringList([])
    def set_manual_suggestions(self,names:list[str])->None:self.manual_suggestions.setStringList(names)
