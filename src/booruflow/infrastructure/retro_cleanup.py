...
def detect_site(path: Path) -> str | None:
    ...
    gelbooru = bool(re.search(r"(?:\(|\b)gelbooru(?:\.com)?(?:\)|\b)", text))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report",
        type=Path,
        help=("Chemin du rapport CSV. Par défaut : results/audit-retroactif-AAAAMMJJ-HHMMSS.csv"),
    )
...
def iter_image_files(roots: Iterable[Path]) -> Iterable[Path]:
    ...
    for root in roots:
        ...
        for path in candidates:
            if path.is_file() and path.suffix.casefold() in IMAGE_EXTENSIONS and path not in seen:
                seen.add(path)
                yield path
...
def write_report(path: Path, matches: Iterable[Match]) -> int:
    ...
    with path.open("w", newline="", encoding="utf-8-sig") as file:
        ...
        writer.writerow(("fichier", "mode", "site_detecte", "tag", "regle_blacklist"))
        for match in rows:
            writer.writerow(
                (
                    str(match.path),
...
def main() -> int:
    ...
    parsed = parse_blacklist(
        args.blacklist.read_text(encoding="utf-8-sig", errors="replace").splitlines()
    )
    print(
    ...
    for file_count, path in enumerate(iter_image_files(args.dossiers), start=1):
        ...
        if file_count == 1 or file_count % 500 == 0:
            print(
                f"Fichiers analysés : {file_count} ; correspondances : {len(matches)}",
                flush=True,
            )
    ...
    print(f"Résultat : {len(unique_files)} fichier(s), {len(matches)} correspondance(s).")
    if matches:
...
def send_to_recycle_bin(paths: Iterable[Path]) -> tuple[bool, str]:
    ...
    iid_file_operation = GUID.from_text("947aab5f-0a5c-4c13-b4d6-4bf7836fc9f8")
    iid_shell_item = GUID.from_text("43826d1e-e718-42ee-bc55-a1e261c37bfe")
    ...
    try:
        hr = ole32.CoCreateInstance(
            ctypes.byref(clsid_file_operation),
            None,
            1,
            ctypes.byref(iid_file_operation),
        ...
        delete_item = method(operation, 18, ctypes.c_long, ctypes.c_void_p, ctypes.c_void_p)
