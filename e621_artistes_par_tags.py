...
def main() -> int:
    ...
    ignored_artists, _ignored_queries, _ignore_path = load_ignore_file(
        args.ignore, artist_names
    )
    available = artist_names - blacklisted - ignored_artists
    args.sortie.mkdir(parents=True, exist_ok=True)
    cache = BooruCache(
        args.sortie / f"cache_e621_{entity.key}.sqlite", f"e621:{entity.key}"
    )
    cumulative_path = args.sortie / f"progression_cumulative_e621_{entity.key}.json"
    state: dict[str, Any] = {
        "queries": args.requetes, "entity_type": entity.key,
        "next_page": 1, "data": {},
    }
    ...
    for query in args.requetes:
        ...
        cached_counts = cache.observed_artist_counts(
            normalize_query(query), entity.e621_post_key
        )
        ...
        candidates = cache.candidates_from_same_or_stricter_queries(
            normalize_query(query)
        )
    ...
    state["next_page"] = (
        args.page_debut + args.pages if fetched_any_page else args.page_debut
    )
    ...
    with (args.sortie / f"classement_{entity.plural}.csv").open(
        ...
        writer = csv.DictWriter(file, fieldnames=list(rows[0]) if rows else [
            "requete", "artiste", "posts_correspondants_observes",
            "posts_totaux_export_e621", "part_observee_pct",
        ], delimiter=";")
        writer.writeheader()
    ...
    if args.cache_check == "none":
        ...
        print(
            f"Cache e621 : {cache.path} "
            f"(intégrité : {cache.check(args.cache_check)})"
        )
    cache.close()
    return 0
