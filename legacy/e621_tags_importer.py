...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--db", type=Path, default=Path("e621_tags.db"), help="Base SQLite cible."
    )
    parser.add_argument(
        "--cache-dir",
        type=Path,
        ...
        with temporary.open(mode) as file:
            while chunk := response.read(1024 * 1024):
                ...
                if now - last_report >= 1:
                    ...
                    print(
                        f"\rTéléchargement : {percent:6.2f}% "
                        f"({speed:5.1f} Mio/s)",
                        end="",
                        flush=True,
                    )
            ...
            for row in reader:
                ...
                if len(batch) >= 10_000:
                    connection.executemany(
                        "INSERT INTO tags_import VALUES(?,?,?,?,?,?,?)", batch
                    )
                    inserted += len(batch)
                    batch.clear()
                    elapsed = max(time.monotonic() - started, 0.001)
                    print(
                        f"\rImport : {inserted:,} tags "
                        f"({inserted / elapsed:,.0f}/s)",
                        end="",
                        flush=True,
                    )
            if batch:
