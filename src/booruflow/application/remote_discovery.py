...
def dominant_source_artists(
    repository: ImageAnalysisRepository, item_ids: tuple[int, ...] | list[int]
) -> set[tuple[str, str]]:
    counts = {}
    for item_id in item_ids:
        for site, tag in set(repository.item_artist_identities(item_id)):
            counts[(site, tag.casefold())] = counts.get((site, tag.casefold()), 0) + 1
    return {
        identity
        for identity, count in counts.items()
        if len(item_ids) == 1 or count / max(1, len(item_ids)) >= 0.75
    }
...
class RemoteDiscoveryService:
    def __init__(
        ...
        encoder: Callable[[list[int]], dict],
        *,
        ...
        self.profiles = ArtistProfileService(repository)
                    ...
                    progress(
                        {
                            ...
                            "role": phase,
                            "artist": artist,
                            "post_ref": f"{site}:{post_id}" if post_id else "",
                            "attempt": attempt,
                            "attempt_total": len(self.retry_delays) + 1,
                            "delay": delay,
                            "network_errors": 1,
                        }
                    )
        ...
        for tag in candidates:
            ...
            frequency = int(
                self.repository.connection.execute(
                    "SELECT COUNT(DISTINCT item_id) FROM source_tags WHERE tag_name=? COLLATE NOCASE",
                    (tag,),
                ).fetchone()[0]
            )
            values.append((frequency, tag))
        return [
            tag
            for _frequency, tag in sorted(
                values, key=lambda value: (value[0], value[1].casefold())
            )[:limit]
        ]
    ...
    def discover(
        ...
        backend: str = "author_id_embedding",
        mode: str = "normal",
        *,
        progress: Callable[[dict], None] | None = None,
        cancelled: Callable[[], bool] | None = None,
        only_new: bool = False,
    ) -> list[RemoteDiscoveryResult]:
        candidate_budget, sample_budget = MODE_BUDGETS.get(mode, MODE_BUDGETS["normal"])
        tags = self.distinctive_tags(query.item_ids)
        identities = []
        queries_sent = posts_received = 0
        images_ignored = network_errors = 0
        source_artists = dominant_source_artists(self.repository, query.item_ids)
        if progress:
            progress(
                {
                    "phase": "distinctive_tags",
                    "tags": len(tags),
                    "queries": 0,
                    "posts": 0,
                    "artists": 0,
        ...
        if not tags:
            if progress:
                progress(
                    {
                        "phase": "finished",
                        "reason": "Aucun tag distinctif exploitable",
                        "tags": 0,
                        "queries": 0,
                        "posts": 0,
                        "artists": 0,
        ...
        for site, provider in self.providers.items():
            ...
            for discovery_tags in queries:
                ...
                try:
                    found = self._retry(
                        lambda provider=provider, discovery_tags=discovery_tags, query_count=query_count: (
                            provider.discover_candidates(
                                discovery_tags, max(1, per_site // query_count)
                            )
                        ),
                        site=site,
                ...
                for tag in found:
                    ...
                    ) not in source_artists and identity not in identities:
                        identities.append(identity)
                if self.throttle_seconds:
                    sleep(self.throttle_seconds)
            identities = identities[:candidate_budget]
        original_profiles = {
            identity: self.profiles.get_profile(identity) for identity in identities
        }
        known = sum(value is not None for value in original_profiles.values())
        if only_new:
            identities = [
                identity for identity in identities if original_profiles[identity] is None
            ]
        if progress:
            progress(
                {
                    ...
                    "queries": queries_sent,
                    "posts": posts_received,
                    "candidates": len(original_profiles),
                    "artists": len(original_profiles),
                    "known": known,
                    "new": len(original_profiles) - known,
                    "filtered": len(identities),
                    "images": 0,
                    "profiles": 0,
                    "evaluated": 0,
                }
        ...
        if not identities:
            reason = (
                "Tous les candidats ont été filtrés"
                if original_profiles
                else "Impossible d’extraire les artistes"
                if posts_received
                else "Aucun post reçu"
            )
            if progress:
                progress(
                    {
            ...
            return []
        sources = ImageSourceService(
            self.repository, self.pixels.directory, bytes_fetcher=self.pixels.bytes_fetcher
        )
        images = 0
        for processed, identity in enumerate(identities, 1):
            ...
            if profile is None:
                ...
                try:
                    sampled = self._retry(
                        lambda identity=identity: self.providers[identity.site].sample_artist_posts(
                            identity.tag, sample_budget
                        ),
                        site=identity.site,
                        phase="candidate_sample",
                        artist=identity.tag,
                        progress=progress,
                    )
                except Exception as exc:  # noqa: BLE001 - external provider boundary
                ...
                for post in sampled:
                    ...
                    try:
                        ...
                        # A deleted post, one without an image, or another unusable result
                        # must not interrupt the entire remote-discovery run.
                        images_ignored += 1
                        if progress:
                            progress(
                                {
                                    ...
                                    "role": "candidate_sample",
                                    "artist": identity.tag,
                                    "post_ref": f"{identity.site}:{post.post_id}",
                                    "result": str(exc),
                    ...
                    images += 1
                if item_ids:
                    self.encoder(list(dict.fromkeys(item_ids)))
                    self.profiles.build_profile(identity, force=True)
                if self.throttle_seconds:
                    sleep(self.throttle_seconds)
        ...
        rankings = {
            row.artist: row
            for row in self.profiles.rank_artists_for_query(query, backend, limit=100000)
        }
        results = []
        for identity in identities:
            ranking = rankings.get(identity)
            profile = self.profiles.get_profile(identity)
            if ranking and profile:
                results.append(
                    RemoteDiscoveryResult(
                        identity,
                        ranking.centroid_similarity,
                        profile.image_count,
                        original_profiles[identity] is None,
                        self.repository.artist_collection_state(identity.site, identity.tag),
                    )
                )
        ...
        if progress:
            progress(
                {
                    "phase": "finished",
                    "reason": "" if results else "Aucune image candidate exploitable",
                    "tags": len(tags),
                    "queries": queries_sent,
                    "posts": posts_received,
                    "artists": len(original_profiles),
