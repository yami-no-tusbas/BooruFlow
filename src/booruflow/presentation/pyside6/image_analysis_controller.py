...
class SourcePreparationWorker(QThread):
    ...
    def __init__(
        ...
        super().__init__()
        self.database = database; self.cache = cache; self.local_paths = local_paths
        self.download_prefetch = download_prefetch; self.credentials = credentials
    ...
    def run(self) -> None:
        ...
        with ImageAnalysisRepository(self.database) as repository:
            ...
            lookup = (
                ...
                if self.gelbooru_tag_database else None
            )
            providers = {
            ...
            for path in self.local_paths:
                try:
                    result = sources.add_local_with_result(path); item_id = result.item_id
                    outcomes[result.outcome] = outcomes.get(result.outcome, 0) + 1
                    if result.outcome != "new": reused_ids.append(item_id)
                ...
                if detected:
                    try:
                        ...
                        enrichment_errors.append(
                            f"{detected.site} #{detected.post_id}: {exc}"
                        )
            for item_id in repository.claim_sources_to_resolve(self.download_prefetch):
                ...
                try:
                    ...
                    repository.fail_source(item_id, str(exc)); errors.append(str(exc))
...
class DroppedSourceScanWorker(QThread):
    ...
    def __init__(self, sources: list[Path]) -> None:
        super().__init__(); self.sources = sources

    def run(self) -> None:
        from PIL import Image
        extensions = {value.casefold() for value in Image.registered_extensions()}
        result: list[str] = []; seen: set[str] = set(); ignored = 0
        def append(path: Path) -> None:
            nonlocal ignored
            try: key = os.path.normcase(str(path.resolve(strict=False)))
            except OSError: key = os.path.normcase(str(path.absolute()))
            if key in seen: ignored += 1; return
            seen.add(key); result.append(str(path))
                ...
                try:
                    for directory, subdirectories, files in os.walk(source):
                        ...
                        for name in sorted(files,key=str.casefold):
                            ...
                            if path.suffix.casefold() in extensions: append(path)
                            else: ignored += 1
                except OSError: ignored += 1
            elif source.is_file(): append(source)
            else: ignored += 1
...
class ImageAnalysisController(QObject):
    ...
    def __init__(
        ...
        self.project_root = project_root; self.python_executable = python_executable
        self.page = page; self.settings = settings; self.credentials = credentials; self.log = log
        ...
        stale = (datetime.now(UTC) - timedelta(seconds=self.stale_seconds)).isoformat(timespec="seconds")
        ...
        self.drop_ignored = 0; self.drop_expected = 0
        ...
        page.set_display_threshold(float(
            settings.get("image_analysis_wd14_display_threshold", 0.30)
        ))
        if auto_start_worker: self.start_worker()
        else: self.page.worker_state.setText(self.page.catalog.text("image_analysis.worker_stopped"))
        self.refresh()
    ...
    def _drop_scan_finished(self, paths: list[str], ignored: int) -> None:
        if self.drop_scan_worker:
            self.drop_scan_worker.deleteLater(); self.drop_scan_worker = None
        threshold = int(self.settings.get("image_analysis_drop_confirmation_threshold", 250))
        if len(paths) > threshold:
            answer = QMessageBox.question(
                self.page, "Ajouter les images",
                f"{len(paths)} images détectées.\nLes ajouter à Image Analysis ?",
            )
            if answer != QMessageBox.StandardButton.Yes:
                self.page.drop_status.setText("Import annulé"); return
        self.drop_ignored = ignored; self.drop_expected = len(paths)
        if not paths:
    ...
    def add_remote_ids(
        self, site: str, post_ids: list[str], *, priority: int = 0
    ) -> list[int]:
    ...
    def _start_source_preparation(self) -> None:
        ...
        gel_path = str(self.settings.get("gelbooru_database", ""))
        self.source_worker = SourcePreparationWorker(
            self.database, self.cache, local, self.policy.download_prefetch,
            self.credentials(), Path(gel_path) if gel_path else None,
        )
    ...
    def _sources_finished(
        self, errors: list[str], enrichment_errors: list[str],
        outcomes: dict[str, int], reused_ids: list[int], events: list[tuple[str, str, str]],
    ) -> None:
        for level, context, message in events:
            self.log(log_event("Metadata", message, level=level, context=context))
        for error in errors: self.log(log_event("ImageAnalysis", error, level="ERROR"))
        for error in enrichment_errors:
            self.log(log_event("Metadata", error, level="WARNING"))
        if self.source_worker: self.source_worker.deleteLater(); self.source_worker = None
        if self.pending_local_paths or self.repository.queue_counts().get("unresolved", 0):
            ...
            self.drop_expected = 0; self.drop_ignored = 0
        self.refresh()
        self.log(log_event(
            "ImageAnalysis", f"Source preparation finished; errors={len(errors)}"
        ))
        if len(reused_ids) == 1:
            ...
            message = (
                ...
                "Image déjà connue — actuellement ignorée"
                if state == "skipped" else "Image déjà présente dans la file"
            )
    ...
    def start_worker(self) -> None:
        ...
        environment.insert("NO_COLOR", "1"); environment.insert("FORCE_COLOR", "0")
        environment.insert("TERM", "dumb")
        self.process.setProcessEnvironment(environment); self.process.setWorkingDirectory(str(self.project_root))
        arguments = [
            "-u", "-m", "booruflow.worker.image_analysis_bootstrap", "--database", str(self.database),
            "--analysis-prefetch", str(self.policy.analysis_prefetch),
            "--heartbeat-interval", str(self.settings.get("image_analysis_worker_heartbeat_interval", 2)),
            "--worker-recycle-after", str(
                self.settings.get("image_analysis_worker_recycle_after", 100)
            ),
            "--poll-interval", "1.0",
            "--parent-pid", str(os.getpid()),
            "--session-id", self._worker_session_id,
        ]
        if bool(self.settings.get("image_analysis_wd14_enabled", True)):
            ...
            arguments.extend([
                "--wd14-enabled", "--wd14-model-directory", str(model_directory),
                "--wd14-model-id", str(self.settings.get(
                    "image_analysis_wd14_model_id", "SmilingWolf/wd-vit-tagger-v3"
                )),
                "--wd14-store-threshold", str(self.settings.get(
    ...
    def _handle_worker_line(self, line: str) -> None:
        ...
        protocol_prefixes = (
            "READY ", "WD14_RUNTIME ", "WD14_READY ", "WD14_UNAVAILABLE ",
            "WD14_OOM ", "RECYCLE ",
        )
        if clean.startswith("["):
            self.log(clean)
    ...
    def install_wd14(self) -> None:
        if self.model_process.state() != QProcess.ProcessState.NotRunning:
            return
        directory = Path(str(self.settings.get(
            "image_analysis_wd14_model_directory",
            self.project_root / "var" / "models" / "image_analysis" / "wd-vit-tagger-v3",
        )))
        answer = QMessageBox.question(
            self.page, "Installer WD14",
        ...
        self.model_process.start(self.python_executable, [
            "-u", "-m", "booruflow.cli.wd14_model", "install",
            "--directory", str(directory), "--model-id", str(self.settings.get(
                "image_analysis_wd14_model_id", "SmilingWolf/wd-vit-tagger-v3"
            )),
        ])
    ...
    def install_gpu_runtime(self) -> None:
        if self.model_process.state() != QProcess.ProcessState.NotRunning:
            return
        answer = QMessageBox.question(
            self.page, "Installer le runtime GPU",
        ...
        self.model_process.setWorkingDirectory(str(self.project_root))
        self.model_process.start(self.python_executable, [
            "-u", "-m", "pip", "install", "onnxruntime-gpu[cuda,cudnn]>=1.27,<2",
        ])
    ...
    def _model_finished(self, code: int, _status) -> None:
        ...
        if code == 0:
            label = "Runtime GPU installé" if self._install_operation == "runtime" else "WD14 installé"
    ...
    def refresh(self) -> None:
        ...
        if not self._page_active:
            ...
            return
        stale = (
            datetime.now(UTC) - timedelta(seconds=self.stale_seconds)
        ...
        self.page.pipeline.setText(self.page.catalog.text(
            "image_analysis.pipeline", unresolved=status.unresolved, resolved=status.resolved,
            pending=status.pending, processing=status.processing,
            ready=status.ready_for_review, failed=status.failed,
        ))
        if self.current is None:
            self.current = self.workflow.next_for_review()
            if self.current: self._show_current()
    ...
    def set_page_active(self,active:bool)->None:
        was_active=self._page_active;self._page_active=active
        if active and (not was_active or self._page_dirty):self._page_dirty=False;self._last_queue_signature=None;QTimer.singleShot(0,self.refresh)

    def _log_slow_refresh(self,elapsed_ms:float,detail:str)->None:
        now=time.monotonic()
        if elapsed_ms<150:
            self.log(log_event("UI",f"Image Analysis refresh slow: {elapsed_ms:.0f} ms; {detail}",level="DEBUG"));return
        if elapsed_ms>=500 or now-self._slow_refresh_last>=10:
            samples=[*self._slow_refresh_samples,elapsed_ms];level="ERROR" if elapsed_ms>=2000 else "WARNING";message=(f"Image Analysis refresh remained slow: count={len(samples)} avg={sum(samples)/len(samples):.0f} ms max={max(samples):.0f} ms; {detail}" if self._slow_refresh_samples else f"Image Analysis refresh slow: {elapsed_ms:.0f} ms; {detail}");self.log(log_event("UI",message,level=level));self._slow_refresh_last=now;self._slow_refresh_samples.clear()
        else:self._slow_refresh_samples.append(elapsed_ms)

    def _show_current(self) -> None:
        self.page.show_review(
            self.current, self.repository.source_tags(self.current.id),
    ...
    def complete(self) -> None:
        ...
        if current is None or current.state.value != "ready_for_review": return
        try: self.current = self.repository.activate_review(current.id)
        except (ValueError, KeyError) as exc: self.log(f"Image Analysis: {exc}"); return
    ...
    def open_item(self, item_id: int) -> None:
        ...
        if item.state.value == "ready_for_review":
            try: item = self.repository.activate_review(item_id)
            except (ValueError, KeyError) as exc:
                self.log(f"Image Analysis: {exc}"); return
        self.current = item
        self._show_current()
        self.refresh()
    ...
    def clean_queue(self, mode: str) -> None:
        ...
        try: changed, retained = self.repository.clean_queue(mode)
        ...
        if self.current and not self.repository.item_queue_visible(self.current.id):
            ...
            if self.current: self._show_current()
            else: self.page.show_review(None, (), [], None)
        self.page.drop_status.setText(
    ...
    def retry(self, item_id: int) -> None:
        try: self.workflow.retry(item_id)
        except (ValueError, KeyError) as exc: self.log(f"Image Analysis: {exc}")
        self.refresh()

    def add_manual_tag(self, name: str) -> None:
        if not self.current: return
        try:
            self.workflow.add_manual_tag(self.current.id, name); self.page.manual_tag.clear()
            self._show_current()
        except ValueError as exc: self.log(f"Image Analysis: {exc}")
