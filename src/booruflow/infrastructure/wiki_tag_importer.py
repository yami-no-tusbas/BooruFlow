...
# Network fallbacks deliberately tolerate heterogeneous remote and parser failures.
...
def _clean_heading(value: str) -> str:
    ...
    value = re.sub(r"\[\[([^]]+)]]", r"\1", value)
    value = re.sub(r"\[/?(?:b|i|u|s)]", "", value, flags=re.IGNORECASE)
...
def parse_e621_group(body: str) -> dict:
    ...
    for raw in body.splitlines():
        ...
        if not line:
            continue
        heading = re.match(r"h([1-6])\.\s*(.+)", line, re.IGNORECASE)
        if heading:
            ...
            lower_headings = [
                ...
                if old_level < level and heading_containers.get(old_level) is container_node
            ]
            parent = heading_nodes[max(lower_headings)] if lower_headings else container_node
        ...
        if skip_heading_level is not None:
            continue
        section = re.match(r"\[section(?:(?:,expanded)?=([^]]+))?]", line, re.IGNORECASE)
        if section:
...
class _GelbooruWikiParser(HTMLParser):
    ...
    def handle_data(self, data: str) -> None:
        ...
        if (
            self.current_href
            and "page=wiki" in self.current_href
            and "search=" in self.current_href
        ):
...
def parse_gelbooru_group(source: str) -> tuple[dict, str]:
    marker = re.search(r"<h2[^>]*>\s*Now Viewing:", source, re.IGNORECASE)
    if not marker:
        return {}, ""
...
def parse_pasted_tag_list(source: str) -> dict:
    ...
    nonempty_depths = [indentation_depth(raw) for raw in raw_lines if raw.strip()]
    base_depth = min(nonempty_depths, default=0)
    indentation_mode = any(depth > base_depth for depth in nonempty_depths)
    effective_depths = [max(0, indentation_depth(raw) - base_depth) for raw in raw_lines]
    ...
    for line_index, raw in enumerate(raw_lines):
        ...
        content = re.sub(r"\[/?[a-z][^]]*]", "", content, flags=re.IGNORECASE).strip()
        content = content.split("//", 1)[0].strip()
        if not has_children:
            content = re.split(r"\s+/\s+", content, maxsplit=1)[0].strip()
        is_heading = not marker and (wrapped_header or content.endswith(":") or markdown_heading)
        if is_heading:
...
def analyze_pasted_tag_list(source: str) -> tuple[dict, dict]:
    ...
    normalized = [(number, max(0, depth - base_depth), text) for number, depth, text in rows]
    jumps = []
...
def import_catalogues(progress=None) -> dict:
    ...
    queue = sorted(
        {tag for tag in iter_tags(boards["e621"]) if tag.casefold().startswith("tag_group:")},
        key=str.casefold,
    )
    ...
    def expand_e621_groups(node, ancestry: frozenset[str]) -> None:
        ...
        if (
            normalized.startswith("tag_group:")
            and normalized in page_trees
            and normalized not in ancestry
        ):
    ...
    def structural_tags(node) -> set[str]:
        ...
        children = [
            child
            for key, child in node.items()
            if key not in {"__tag__", "__tags__", "__manual__"} and isinstance(child, (dict, list))
        ]
    ...
    while (implication_queue or ordinary_queue) and len(ordinary_visited) < 400:
        ...
        while (
            ...
            and len(batch) < 8
            and len(ordinary_visited) + len(batch) < 400
        ):
            title = implication_queue.pop(0) if implication_queue else ordinary_queue.pop(0)
            normalized = title.casefold()
        ...
        for title, _fetch_page in batch:
            ...
            if page is not None:
                ...
                wiki_referenced_tags.update(tag.casefold() for tag in iter_tags(parsed))
            ...
            if progress:
                progress(f"e621 wiki/implications {len(ordinary_visited)}/400 : {title}")
    ...
    def expand_e621_details(node, ancestry: frozenset[str]) -> None:
        ...
        for key, child in list(node.items()):
            if key not in {"__tag__", "__tags__", "__manual__"}:
                child_tag = (
                    str(child.get("__tag__", "")).casefold() if isinstance(child, dict) else ""
                )
    ...
    for tag in iter_tags(boards["e621"]):
        ...
        entry.setdefault(
            "wiki_url",
            f"https://e621.net/wiki_pages/show_or_new?title={urllib.parse.quote(tag)}",
        )
    if progress:
        progress(
        ...
        for title, _url in batch:
            ...
            for tag in iter_tags(group_tree):
                metadata["gelbooru"].setdefault(
                    tag,
                    {
                        "wiki_url": "https://gelbooru.com/index.php?"
                        + urllib.parse.urlencode({"page": "wiki", "s": "list", "search": tag})
                    },
                )
                if (
                    tag.casefold().startswith(("tag_group:", "list_of_"))
                    and tag.casefold() not in visited
                ):
                    queue.append(tag)
...
def merge_catalogues(document: dict, imported: dict) -> dict[str, int]:
    ...
    for board, groups in imported["boards"].items():
        ...
        def filter_excluded(node, excluded_tags=excluded):
            if isinstance(node, list):
                return [tag for tag in node if tag not in excluded_tags]
            if isinstance(node, dict):
                result = {}
                for key, child in node.items():
                    if key == "__tag__":
                        if child not in excluded_tags:
                            result[key] = child
                    else:
                        result[key] = filter_excluded(child, excluded_tags)
                return result
            return node
        ...
        def preserve_manual(existing, refreshed):
            ...
            if manual_tags:
                refreshed["__tags__"] = list(
                    dict.fromkeys(list(refreshed.get("__tags__", [])) + list(manual_tags))
                )
...
def tag_definition_details(board: str, tag: str, wiki_url: str = "") -> tuple[str, str, list[str]]:
    if board == "e621":
        ...
        page = (
            json.loads(_get(f"https://e621.net/wiki_pages/{page_id.group(1)}.json"))
            if page_id
            else _e621_page(tag)
        )
