...
class BooruCache:
    ...
    def _initialize(self) -> None:
        if self.site.startswith("gelbooru:"):
            ...
            if "gel_queries" in existing_tables:
                compact_rows = int(
                    self.connection.execute(
                        "SELECT COUNT(*) FROM gel_queries"
                    ).fetchone()[0]
                )
            if legacy_rows and not compact_rows:
                self.connection.close()
                raise RuntimeError(
                    f"Cache Gelbooru v1 à migrer avant utilisation : {self.path}"
                )
    ...
    def store_posts(
        ...
        for post in posts:
            ...
            self.connection.execute(
                "DELETE FROM post_tags WHERE post_id=?", (post_id_text,)
            )
            self.connection.executemany(
                "INSERT OR IGNORE INTO post_tags(post_id,tag,category) VALUES(?,?,?)",
                (
                    (post_id_text, tag, str(tag_categories.get(tag, "")))
                    for tag in tags
                ),
            )
            self.connection.execute(
    ...
    def get_query_count(
        self, query_key: str, artist: str, max_age_days: int
    ) -> int | None:
        row = self.connection.execute(
            """
    ...
    def observed_artist_counts(
        self, query_key: str, category: str = "artist"
    ) -> dict[str, int]:
    ...
    def processed_queries(self) -> set[str]:
        return {
            str(row[0])
            for row in self.connection.execute(
                "SELECT query_key FROM processed_queries"
            )
        }
    ...
    def replace_query_results(
        ...
        with self.connection:
            self.connection.execute(
                "DELETE FROM query_results WHERE query_key=?", (key,)
            )
            self.connection.executemany(
                """
                ...
                """,
                (
                    (key, name, int(hits), int(scanned_posts), now)
