...
class ImageAnalysisRepository:
    ...
    def _migrate(self) -> None:
        ...
        if version == 12:
            migration = (
                files("booruflow.infrastructure.schema")
                .joinpath("image_analysis_v13.sql")
                .read_text(encoding="utf-8")
            )
            with self.connection:
                self.connection.executescript(migration)
        ...
        if version == 13:
            migration = (
                files("booruflow.infrastructure.schema")
                .joinpath("image_analysis_v14.sql")
                .read_text(encoding="utf-8")
            )
            with self.connection:
                self.connection.executescript(migration)
        ...
        if version == 14:
            migration = (
                files("booruflow.infrastructure.schema")
                .joinpath("image_analysis_v15.sql")
                .read_text(encoding="utf-8")
            )
            with self.connection:
                self.connection.executescript(migration)
        ...
        if version == 15:
            migration = (
                files("booruflow.infrastructure.schema")
                .joinpath("image_analysis_v16.sql")
                .read_text(encoding="utf-8")
            )
            with self.connection:
                self.connection.executescript(migration)
        ...
        if version == 16:
            migration = (
                files("booruflow.infrastructure.schema")
                .joinpath("image_analysis_v17.sql")
                .read_text(encoding="utf-8")
            )
            with self.connection:
                self.connection.executescript(migration)
        ...
        if version == 17:
            migration = (
                files("booruflow.infrastructure.schema")
                .joinpath("image_analysis_v18.sql")
                .read_text(encoding="utf-8")
            )
            with self.connection:
                self.connection.executescript(migration)
        ...
        if version == 18:
            ...
            with self.connection:
                if "batch_visible" not in columns:
                    migration = (
                        files("booruflow.infrastructure.schema")
                        .joinpath("image_analysis_v19.sql")
                        .read_text(encoding="utf-8")
                    )
                    self.connection.executescript(migration)
                else:
        ...
        if version == 19:
            migration = (
                files("booruflow.infrastructure.schema")
                .joinpath("image_analysis_v20.sql")
                .read_text(encoding="utf-8")
            )
            with self.connection:
                self.connection.executescript(migration)
    ...
    def replace_source_metadata(
        ...
        with self.connection:
            self.connection.execute(
                "DELETE FROM source_tags WHERE item_id=? AND site=?", (item_id, site)
            )
            self.connection.executemany(
                """INSERT INTO source_tags(item_id,site,tag_name,category,fetched_at)
                   VALUES(?,?,?,?,?)""",
    ...
    def cache_post_metadata(
        ...
        serialized_tags = json.dumps(
            [{"name": tag.name, "category": tag.category} for tag in tags], ensure_ascii=False
        )
        with self.connection:
            self.connection.execute(
    ...
    def artist_associations(self, item_id: int) -> list[dict]:
        return [
            dict(row)
            for row in self.connection.execute(
                "SELECT site,artist_tag,provenance FROM item_artists WHERE item_id=? ORDER BY site,artist_tag,provenance",
                (item_id,),
            )
        ]
    ...
    def item_artist_identities(self, item_id: int) -> tuple[tuple[str, str], ...]:
        return tuple(
            (str(row[0]), str(row[1]))
            for row in self.connection.execute(
                "SELECT site,artist_tag FROM item_artists WHERE item_id=? ORDER BY site,artist_tag",
                (item_id,),
            )
        )
    ...
    def set_cached_path(self, item_id: int, path: Path | None) -> None:
        with self.connection:
            self.connection.execute(
                "UPDATE analysis_items SET cached_path=?,updated_at=? WHERE id=?",
                (str(path) if path else None, utc_now(), item_id),
            )

    def record_filename_metadata(
        self,
        item_id: int,
        path: Path,
        artist: str,
    ...
    def assign_artist(
        ...
        if not unique or not tag or site not in {"local", "gelbooru", "e621"}:
            return 0
        with self.connection:
    ...
    def repair_structured_artist_associations(self) -> dict[str, int]:
        ...
        for row in rows:
            try:
                artists = tuple(
                    str(value).strip()
                    for value in json.loads(str(row["artist_tags_json"]))
                    if str(value).strip()
                )
    ...
    def unassigned_artist_diagnostics(self) -> list[dict]:
        ...
        for item in items:
            ...
            source_tags = [
                {"site": tag.source.value, "name": tag.name, "category": tag.category}
                for tag in self.source_tags(int(item["id"]))
            ]
            metadata = []
            for source in remote:
                cached = self.cached_post_metadata(str(source["site"]), str(source["post_id"]))
                if cached:
                    metadata.append(dict(cached))
            artists = []
            for cached in metadata:
                try:
                    artists.extend(
                        str(value) for value in json.loads(str(cached["artist_tags_json"]))
                    )
                except (TypeError, ValueError):
                    pass
            ...
            if remote and not metadata_available:
                reason = "remote_metadata_not_resolved"
            elif remote and artists:
                reason = "remote_artist_tag_not_extracted"
            elif remote:
                reason = "remote_metadata_without_artist"
            elif provenances and all(row.get("kind") == "local_file" for row in provenances):
                reason = "local_only_no_artist_metadata"
            else:
                reason = "unknown"
            result.append(
                {
                    "item_id": int(item["id"]),
                    "sha": str(item["content_sha256"] or "")[:12],
                    "cached_path": str(item["cached_path"]),
                    "provenances": provenances,
                    "source_tags": source_tags,
                    "metadata_available": metadata_available,
                    "metadata_cache_available": bool(metadata),
                    "metadata_artists": tuple(dict.fromkeys(artists)),
                    "reason": reason,
                }
            )
        return result
    ...
    def scheduler_diagnostic(self, analysis_prefetch: int) -> dict[str, object]:
        ...
        reason = (
            ...
            if interactive
                ...
                "prefetch_limit"
                if eligible and ahead >= analysis_prefetch
                else "eligible_pending"
                if eligible
                else "no_eligible_pending_item"
            )
        )
        exclusions = [
            dict(row)
        ...
        return {
            ...
            "ready_ahead": int(counts[2] or 0),
            "review_active": int(counts[3] or 0),
            "analysis_prefetch": analysis_prefetch,
            "eligible": eligible,
    ...
    def tagging_pool(
        self, item_ids: list[int] | tuple[int, ...] | None = None
    ) -> list[sqlite3.Row]:
    ...
    def add_to_tagging_pool(self, item_ids: list[int] | tuple[int, ...], source: str) -> int:
        ...
        with self.connection:
            self.connection.executemany(
                "INSERT OR IGNORE INTO tagging_pool_items(item_id,source,added_at) VALUES(?,?,?)",
                ((item_id, source, now) for item_id in values),
            )
        return len(values)
    ...
    def save_review_batch_entry(
        ...
        *,
        ...
        has_e621_delta = bool(self._stable_tags(additions) or self._stable_tags(removals))
        default_state = (
            PublishState.PENDING_PUBLISH
            if post_id is not None
            and (site == "gelbooru" or (site == "e621" and has_e621_delta))
    ...
    def batch_entry(self, item_id: int) -> dict[str, object] | None:
        ...
        return {
            ...
            "published_final_tags": (
                ...
                if row["published_final_tags_json"] is not None
                else None
            ),
        }
    ...
    def reviewed_remote_post_ids(self, site: str = "gelbooru") -> set[int]:
        rows = self.connection.execute(
            "SELECT post_id FROM tagging_review_batch_entries "
            "WHERE site=? AND post_id IS NOT NULL",
            (site,),
        )
        ...
        with self.connection:
            changed = self.connection.execute(
                ...
                (
                    ...
                    utc_now(),
                    item_id,
                    PublishState.PENDING_PUBLISH.value,
                ),
            ).rowcount
        if not changed:
    ...
    def next_tagging_pool_item(
        ...
        conditions = [
            "p.item_id<>?",
            "b.item_id IS NULL",
            "i.state IN ('ready_for_review','reviewed')",
        ]
        if scope == "remote":
    ...
    def activate_review(self, item_id: int) -> AnalysisItem:
        ...
        try:
            ...
            self.connection.execute(
                "UPDATE analysis_items SET review_active=0 WHERE review_active=1"
            )
            self.connection.execute(
                "UPDATE analysis_items SET review_active=1,updated_at=? WHERE id=?",
                (utc_now(), item_id),
    ...
    def reanalyze(self, item_id: int, priority: int = 100) -> None:
        """Recompute automatic backends for one existing item using current settings.

        WD14 observations are derived data, so a manual recovery request replaces
        them. Manual observations and source tags remain intact.
        """
        row = self.get_item(item_id)
        if row is None:
            raise KeyError(item_id)
        if row.state is AnalysisState.PROCESSING:
            raise ValueError("cannot re-analyze an item that is already processing")
        now = utc_now()
        with self.connection:
            self.connection.execute(
                """UPDATE model_runs SET state='stale',finished_at=?,
                   error='Re-analysis requested' WHERE item_id=? AND state='completed'""",
                (now, item_id),
            )
            self.connection.execute(
            "DELETE FROM tag_observations WHERE item_id=? AND source IN ('wd14', 'hydra')",
                (item_id,),
            )
            self.connection.execute(
                """UPDATE analysis_items SET state='pending',analysis_requested=1,
                   priority=MAX(priority,?),review_active=0,processing_started_at=NULL,
                   processing_heartbeat_at=NULL,last_error=NULL,updated_at=? WHERE id=?""",
                (priority, now, item_id),
            )
    ...
    def save_statistics(self, item_id: int, run_id: int, statistics: ColorStatistics) -> None:
        now = utc_now()
        ...
        with self.connection:
            self.connection.execute(
                ...
                (
                    item_id,
                    run_id,
                    statistics.mean_saturation,
                    statistics.mean_luminance,
                    statistics.luminance_stddev,
                    statistics.contrast,
                    statistics.pastel_score,
                    palette,
                ),
            )
            self.connection.execute(
    ...
    def similar_corpus_summary(self) -> dict[str, int]:
        eligible = int(
            self.connection.execute("SELECT COUNT(DISTINCT item_id) FROM item_artists").fetchone()[
                0
            ]
        )
        total = int(
        ...
        return {
            "images_eligible": eligible,
            "images_skipped": max(0, total - eligible),
        ...
        with self.connection:
            self.connection.execute(
                ...
                (
                    ...
                    profile_version,
                    dependency_hash,
                    json.dumps(embedding_versions, sort_keys=True),
                    image_count,
                    profile_json,
                    utc_now(),
                ),
    ...
    def create_library_job(self, job_id: str, roots: list[str]) -> None:
        ...
        with self.connection:
            self.connection.execute(
                "INSERT INTO library_index_jobs(id,roots_json,state,created_at,updated_at) VALUES(?,?,'pending',?,?)",
                (job_id, json.dumps(roots, ensure_ascii=False), now, now),
            )

    def library_job(self, job_id: str) -> dict | None:
        row = self.connection.execute(
            "SELECT * FROM library_index_jobs WHERE id=?", (job_id,)
        ).fetchone()
    ...
    def update_library_job(self, job_id: str, **values) -> None:
        ...
        with self.connection:
            self.connection.execute(
                f"UPDATE library_index_jobs SET {columns} WHERE id=?", (*changes.values(), job_id)
            )
    ...
    def resumable_library_jobs(self) -> list[dict]:
        return [
            dict(row)
            for row in self.connection.execute(
                "SELECT * FROM library_index_jobs WHERE state IN ('pending','running','paused','failed') ORDER BY created_at"
            )
        ]
    ...
    def library_path_processed(self, job_id: str, path: Path) -> bool:
        return bool(
            self.connection.execute(
                "SELECT 1 FROM library_index_paths WHERE job_id=? AND path=?", (job_id, str(path))
            ).fetchone()
        )

    def record_library_path(
        self, job_id: str, path: Path, item_id: int | None, outcome: str
    ) -> None:
        stored = (
            item_id
            if item_id is not None
            and self.connection.execute(
                "SELECT 1 FROM analysis_items WHERE id=?", (item_id,)
            ).fetchone()
            else None
        )
        with self.connection:
            self.connection.execute(
                "INSERT OR REPLACE INTO library_index_paths(job_id,path,item_id,outcome,processed_at) VALUES(?,?,?,?,?)",
                (job_id, str(path), stored, outcome, utc_now()),
            )

    def library_match_counts(self, job_id: str) -> dict[str, int]:
        rows = self.connection.execute(
            "SELECT outcome,COUNT(*) count FROM library_index_paths WHERE job_id=? GROUP BY outcome",
            (job_id,),
        )
    ...
    def local_binary_duplicates(self) -> list[dict]:
        ...
        for row in rows:
            paths = [
                str(value[0])
                for value in self.connection.execute(
                    "SELECT local_path FROM image_provenances WHERE item_id=? AND kind='local_file' ORDER BY local_path",
                    (row["id"],),
                )
            ]
            result.append(
                {
                    ...
                    "thumbnail": str(row["cached_path"] or paths[0]),
                    "paths": paths,
                }
            )
        return result
    ...
    def artist_collection_state(self, site: str, artist_tag: str) -> str:
        local = bool(
            self.connection.execute(
                """SELECT 1 FROM item_artists a JOIN image_provenances p ON p.item_id=a.item_id WHERE a.site=? AND a.artist_tag=? COLLATE NOCASE AND p.kind='local_file' LIMIT 1""",
                (site, artist_tag),
            ).fetchone()
        )
        remote = bool(
            self.connection.execute(
                """SELECT 1 FROM item_artists a JOIN image_provenances p ON p.item_id=a.item_id WHERE a.site=? AND a.artist_tag=? COLLATE NOCASE AND p.site IS NOT NULL LIMIT 1""",
                (site, artist_tag),
            ).fetchone()
        )
        return (
    ...
    def preview_remote_profile_purge(self, unused_before: str) -> dict:
        identities = [
            (str(row[0]), str(row[1]))
            for row in self.connection.execute(
                """SELECT r.site,r.artist_tag FROM remote_artist_state r WHERE r.protected=0 AND COALESCE(r.last_used_at,r.last_seen_at)<? AND NOT EXISTS(SELECT 1 FROM item_artists a JOIN image_provenances p ON p.item_id=a.item_id WHERE a.site=r.site AND a.artist_tag=r.artist_tag COLLATE NOCASE AND p.kind='local_file')""",
                (unused_before,),
            )
        ]
        embeddings = 0
        for site, tag in identities:
            embeddings += int(
                self.connection.execute(
                    "SELECT COUNT(*) FROM embeddings e WHERE EXISTS(SELECT 1 FROM item_artists a WHERE a.item_id=e.item_id AND a.site=? AND a.artist_tag=? COLLATE NOCASE)",
                    (site, tag),
                ).fetchone()[0]
            )
    ...
    def purge_remote_profiles(self, identities: list[tuple[str, str]]) -> dict:
        ...
        with self.connection:
            for site, tag in identities:
                cursor = self.connection.execute(
                    "DELETE FROM artist_profiles WHERE site=? AND artist_tag=? COLLATE NOCASE",
                    (site, tag),
                )
                removed_profiles += max(0, cursor.rowcount)
                item_ids = [
                    int(row[0])
                    for row in self.connection.execute(
                        """SELECT DISTINCT a.item_id FROM item_artists a WHERE a.site=? AND a.artist_tag=? COLLATE NOCASE AND NOT EXISTS(SELECT 1 FROM image_provenances p WHERE p.item_id=a.item_id AND p.kind='local_file')""",
                        (site, tag),
                    )
                ]
                for item_id in item_ids:
                    ...
                    removed_embeddings += max(0, cursor.rowcount)
    ...
    def save_tag_predictions(
        ...
        *,
        ...
        for raw_name, category, confidence in predictions:
            ...
            normalized = canonicalize(raw) if canonicalize is not None else raw.replace(" ", "_")
            normalized = str(normalized).strip().replace(" ", "_")
            rows.append(
                (
                    item_id,
                    run_id,
                    ...
                    category,
                    raw,
                    int(normalized.casefold() in source_names),
                    now,
                )
            )
        with self.connection:
            self.connection.execute("DELETE FROM tag_observations WHERE model_run_id=?", (run_id,))
            self.connection.executemany(
    ...
    def tag_review_summary(
        self, item_id: int, original_tags: list[str] | tuple[str, ...]
    ) -> dict[str, list[str]]:
        decisions = {
            str(row["tag_name"]): str(row["decision"])
            for row in self.connection.execute(
                "SELECT tag_name,decision FROM tag_review_entries WHERE item_id=? AND origin='existing'",
                (item_id,),
            )
        }
        ...
        for _observation_id, observation in self.observations(item_id):
            if observation.decision is DecisionState.ACCEPTED and observation.source in {
                ObservationSource.WD14,
                ObservationSource.HYDRA,
        ...
        return {
            ...
            "additions": additions,
            "removals": removals,
            "final_tags": sorted((set(original) - set(removals)) | set(additions)),
        }
